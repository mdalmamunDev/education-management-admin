<template>
  <div class="bg-1 pb-10">
    ...
    <div class="mb-3 mx-1 flex justify-between items-center">
      ...
      <router-link :to="`/assignment?athleteId=${id}`" class="btn-g py-1 px-2 rounded">Show More</router-link>
    </div>
...
export default {
  ...
  data() {
    return {
      headers: ["Task", "Date", "Time", "Coach", "Status"],
      id: null,
      details: {},
    };
  },
  mounted() {
    ...
    this.fetchData({
      customUrl: 'athlete/details', urlSuffix: this.id,
      callback: (data) => {
        this.details = data;
      }
    })
